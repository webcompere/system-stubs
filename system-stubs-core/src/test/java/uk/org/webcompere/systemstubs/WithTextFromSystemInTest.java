...
class WithTextFromSystemInTest {
    ...
    @Test
    void scanner_can_read_all_lines_when_exception_at_end() throws Exception {
        withTextFromSystemIn("arbitrary", "text")
            .andExceptionThrownOnInputEnd(DUMMY_IO_EXCEPTION)
            .execute(() -> {
    ...
    @Test
    void scanner_cannot_read_beyond_last_line() throws Exception {
        withTextFromSystemIn("arbitrary", "text")
            .execute(() -> {
                Scanner scanner = new Scanner(System.in);
                scanner.nextLine();
                scanner.nextLine();
                assertThatThrownBy(scanner::nextLine)
                    .hasMessage("No line found");
            });
    }

    @Test
    void scanner_fails_with_thrown_exception_when_exception_at_end() throws Exception {
        withTextFromSystemIn("arbitrary", "text")
	...
	void system_in_reads_zero_bytes_even_if_mock_should_throw_IOException_on_input_end() throws Exception {
		withTextFromSystemIn()
			...
			.execute(() -> {
				...
				assertThat(numBytesRead).isEqualTo(0);
			});
	}
